...
function FileListSetting() {
    ...
    const { imageFiles, selDrawImageIndex, selImageIndexes, imageSizes, drawStatus, shapes, selShapeIndex } = state;
    ...
    const onClearSelectClick = () => {
        ...
        const newImageFiles = imageFiles.filter((item, index) => selImageIndexes.indexOf(index) === -1);
        const newShapes = shapes.filter((item, index) => selImageIndexes.indexOf(index) === -1);
        const newImageSizes = imageSizes.filter((item, index) => selImageIndexes.indexOf(index) === -1);
        ...
        dispatch({
            ...
            payload: {
                ...
                drawStatus: selImageIndexes.indexOf(selDrawImageIndex) === -1 ? drawStatus : drawStatusTypes.IDLE,
                shapes: newShapes,
